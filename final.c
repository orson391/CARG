...
void distribute_cards(Card* deck, int deckSize, Player players[], int numPlayers) {
    ...
    for (int i = 0; i < deckSize; i++) {
        ...
        Card *tempCard = (Card *)malloc(sizeof(Card));
        tempCard[cardIndex] = deck[i];
        players[playerIndex].MyCards = tempCard; // Assign card to player
...
void main()
{
    ...
    int first = firstmove(players, MAX_PLAYERS);
